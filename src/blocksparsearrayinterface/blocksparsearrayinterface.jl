...
using DerivableInterfaces: DerivableInterfaces, @interface, AbstractArrayInterface,
    DefaultArrayInterface, interface, permuteddims, zero!
using LinearAlgebra: Adjoint, Transpose
using SparseArraysBase: AbstractSparseArrayInterface, eachstoredindex, getstoredindex,
    getunstoredindex, iperm, perm, storedlength, storedvalues
...
function Base.similar(interface::AbstractBlockSparseArrayInterface, T::Type, ax::Tuple)
    ...
    N = length(ax)
    B = similartype(
        typeof(blockinterface(interface)),
        Type{T},
        Tuple{blockaxistype.(ax)...}
    )
...
function BlockSparseArrayInterface{M, B}(
        ::Val{N}
    ) where {M, B <: AbstractArrayInterface{M}, N}
    B′ = B(Val(N))
